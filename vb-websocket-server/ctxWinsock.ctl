...
Private Declare Function ws_recvfrom Lib "ws2_32" Alias "recvfrom" (ByVal s As Long, ByRef buf As Any, ByVal BufLen As Long, ByVal Flags As Long, ByVal from As Any, ByVal fromlen As Long) As Long
...
Public Property Get pvSocket() As cTlsSocket
...
Public Function recv(ByRef strBuffer As String, Optional ByVal bLength As Long) As Long
...
'  Dim arrBuffer(1 To MAX_BUFFER_LENGTH)   As Byte
  ...
  Dim strTempBuffer                       As String
    ...
    'Call the recv Winsock API function in order to read data from the buffer
             lngBytesReceived = ws_recv(pvSocket.SocketHandle, arrBuffer(0), bLength, 0&)
...
Public Sub SendData(Data As Variant, Optional ByVal codePage As UcsAsyncSocketCodePageEnum = UcsAsyncSocketCodePageEnum.ucsScpAcp)
...
'        m_baSendBuffer = pvSocket.ToTextArray(CStr(Data), ucsScpAcp)
        m_baSendBuffer = pvSocket.ToTextArray(CStr(Data), codePage)
    Case vbByte + vbArray
        m_baSendBuffer = Data
    Case Else
